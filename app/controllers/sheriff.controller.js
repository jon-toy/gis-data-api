...
function parseEditReportCsvFile(req, res, next, folder_name) {
  ...
  var file_name =
    __dirname + "/../../public/" + folder_name + "/" + EDIT_HISTORY_FILENAME;
  console.log(
    "Uploaded sheriff edit history export. Saving as " +
      folder_name +
      "/" +
      data.name
  );
  ...
  fs.readFile(data.path, "utf8", function (err, contents) {
    uploadFileToS3(
      "ruraladdress/" + EDIT_HISTORY_FILENAME,
      contents,
      function (err, data) {
        if (!err) exports.readEditHistoryIntoMemory(data);
...
exports.readEditHistoryIntoMemory = (folder) => {
  ...
  s3.listObjectsV2(params, (err, data) => {
    for (var i = 0; i < data.Contents.length; i++) {
      ...
      var s3Stream = s3
        .getObject({ Bucket: S3_BUCKET_NAME, Key: data.Contents[i].Key })
        .createReadStream();
      readLines(
        s3Stream,
        file,
        (line, fileName, zone_edit_history_parcels) => {
          if (
            line.indexOf("APN") >= 0 &&
            line.indexOf("SITUS") >= 0 &&
            line.indexOf("ROAD") >= 0 &&
            line.indexOf("EDITS") >= 0
          )
            return;
          var account = {}; // Account object to hold the data we're about to read in

          var fields = line.split("\t");
          if (fields[0]) account.apn = fields[0];
          else return;
          if (fields[1]) account.situs = fields[1];
          else return;
          if (fields[2]) account.road = fields[2];
          else return;
          if (fields[3]) account.owner = fields[3];
          else return;
          if (fields[4]) {
            if (fields[4] != "NULL") account.remarks = fields[4];
            else account.remarks = "";
          } else return;
          ...
          while (
            ...
            if (fields[index] != "\r") {
              ...
              if (text.length > 10) {
                ...
                // Length is 10
                var dateStringRaw = fields[index].substring(
                  text.length - 10,
                  text.length
                );
          ...
          // Set in redis
          redis_client.set(
            SHERIFF_EDIT_HISTORY_PREFIX + account.apn,
        ...
        (fileName, zone_edit_history_parcels) => {
          ...
          console.log(
            "Creating zone object for zone: " + fileName.replace(".tsv", "")
          );
          const debug = zone_edit_history_parcels.find(
            (element) => element.apn === "204-61-514"
          );
          console.log("Number of parcels: " + zone_edit_history_parcels.length);

          redis_client.set(
            ZONE_EDIT_HISTORY_PREFIX + fileName.replace(".tsv", ""),
            JSON.stringify(zone_edit_history_parcels)
          );
        }
      );
    }
    ...
    files.forEach((file) => {
      ...
      readLines(
        ...
        (line) => {
          ...
          if (fields[0]) rotation.marker = fields[0];
          else return;
          if (fields[1]) rotation.radians = parseFloat(fields[1]);
          else return;
        ...
        (fileName) => {
          ...
          console.log(
            "Loading rotations for zone: " + fileName.replace(".tsv", "")
          );
          console.log(rotations);

          redis_client.set(
            ZONE_ROTATION_PREFIX + fileName.replace(".tsv", ""),
